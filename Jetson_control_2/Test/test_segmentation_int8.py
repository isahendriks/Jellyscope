...
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))  # Jetson_control_2/ (-> monitoring/ post-move)
...
ROOT_DIR = Path("/media/jellyfish/ssd1/Training data/Binary_classifier")
...
for image_path in image_paths:
    ...
    ### Per-crop candidates -- peak_threshold=0 (not the deployed scorer_threshold) so every
    ### local maximum becomes a scored candidate, giving a continuous peak_val to sweep a
    ### threshold over (mirrors test_segmentation2.py's peak_intensity-as-score convention).
    candidates = segment_core.find_candidates(
        prob_map_small, peak_threshold=0.0, secondary_threshold=0.0, upscale_factor=upscale_factor,
        min_region_size_patches=min_region_size_patches, crop_padding_pixels=crop_padding_pixels,
