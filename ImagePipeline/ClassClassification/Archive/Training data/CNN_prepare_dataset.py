...
# Folder paths
ROOT = r"R:\LU24A1037-Jellyscope\Jellyscope\Training data\Monitoring_training_data"
original_folder = os.path.join(ROOT, "OG_training_data") # Folder containing class subfolders
padded_folder = os.path.join(ROOT, "OG_segments_padded_augmented")                       # Where to save padded images
output_folder = os.path.join(ROOT, "squared_split_training_data")           # Where to save split dataset
...
min_class_count = 7  # Minimum number of images per class before augmentation
...
for i in range(len(class_folders)):
    ...
    # exclude classes with too few images
    if len(image_files) < min_class_count:
        print(f"    Skipping {class_name}, too few images (<{min_class_count})")
        class_rm.append(class_folder)
print("="*60)

for class_folder in class_rm:
    class_folders.remove(class_folder)
...
#%% Cell 5: Pad images and visualize examples
...
for class_folder in class_folders: # sorted(padded_path.iterdir()):
    if class_folder.is_dir():
        class_name = class_folder.name
        class_folder = padded_path / class_name
        image_files = [f for f in class_folder.iterdir() if f.is_file()]
...
print(f"Splitting  {len(class_folders)} classes...\n")

# Process each class
for classes in class_folders:
    ...
    if not image_files:
        print(f"Class {class_idx+1}/{len(class_folders)} ({class_name}): No images found")
        continue
    ...
    print(f"Class {class_idx+1}/{len(class_folders)} ({class_name}): {len(train_files)} train, {len(val_files)} val, {len(test_files)} test")
