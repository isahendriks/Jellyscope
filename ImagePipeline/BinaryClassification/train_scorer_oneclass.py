...
# Create data loader
test_loader_empty = DataLoader(test_tiles_empty, batch_size=batch_size, shuffle=False,
...
ax.grid(alpha=0.3)
plt.tight_layout()
plt.savefig('scorer_oneclass_training.png', dpi=150)
print(f"\nSaved training curves to: scorer_oneclass_training.png")
plt.show()

#%% Save one-class scorer model
scorer_model_name = model_name.replace("vae_model", "scorer_oneclass_model").replace(".pth", ".pth")
