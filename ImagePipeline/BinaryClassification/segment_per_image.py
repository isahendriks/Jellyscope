...
def main():
    ...
    for image_idx, image_path_str in enumerate(images_path, start=1):
        ...
        print(f"[{image_idx}/{len(images_path)}] {image_name}", end="\r")
        ...
        result = image_to_probability_map(
            image=image_gray,
            model=model,
            scorer=scorer,
            ...
            offsets=offsets,
            offsets_norm=offsets_norm,
        )

        if result is None:
            print(f"\n⚠ No tiles generated for {image_name}, skipping")
            continue
        ...
        # Overlapping regions are resolved by keeping the one with the highest peak score.
        candidates = []
        for peak_y_small, peak_x_small, peak_val in maxima_small:
            ...
            # Map small-grid bounds to full-resolution pixel bbox (inclusive of the last cell)
        ...
        # Save crops and metadata for accepted regions
        crop_rows = []
        ...
        print(f"[{image_idx}/{len(images_path)}] {image_name}: {len(crop_rows)} crops found. Total so far: {crop_counter}")

        # delete intermediate variables to free memory
        del image_gray, prob_map_small, maxima_small, candidates, accepted, crop_rows, result
