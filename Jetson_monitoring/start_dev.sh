...
CAMERA_HOST_ADDR="169.254.141.1/16" # /16 to reach the camera's self-assigned link-local (RFC 3927) address anywhere in 169.254.0.0/16, not just this /24
...
# Absolute path to this venv's own python, not `source .../activate` + bare `python` --
# this box has more than one venv (e.g. a generic one with no itala package can already
# be active in the calling shell), and activating a second venv on top doesn't reliably
# win the PATH race. Calling the interpreter directly sidesteps that entirely.
...
for iface in "${IFACES[@]}"; do
  ...
  if "${VENV_PY}" "${SCRIPT_DIR}/list_cameras.py" >/dev/null 2>&1; then
...
"${VENV_PY}" "${SCRIPT_DIR}/list_cameras.py"
