...
# monitoring pipeline (record.py/analyse.py/etc) is a separate, older one
...
VENV="${SCRIPT_DIR}/.venv-jellyscope_on_jetson"
...
python -m pip install numpy pillow matplotlib jupyterlab ipykernel opencv-python kornia scipy pandas scikit-learn onnx onnxscript pyserial pymodbus flask psutil
...
echo "==> Step 11: Install itala sdk for the Itala camera"
python -m pip install /opt/itala-sdk/bindings/itala-1.4.11-cp311-abi3-manylinux2014_aarch64.whl
